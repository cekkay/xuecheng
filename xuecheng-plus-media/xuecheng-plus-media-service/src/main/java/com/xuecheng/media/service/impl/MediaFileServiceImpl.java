...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    // 自己注入自己，来解决非事务方法调用事务方法导致的事务失效问题
    @Autowired
    ...
    public boolean addMediaFilesToMinio(String localFilePath, String mimeType, String bucket, String objectName) {
        try {
            UploadObjectArgs uploadObjectArgs = UploadObjectArgs.builder()
                    .bucket(bucket) // 桶
                    .filename(localFilePath) // 制定本地文件路径
                    .object(objectName) // 对象名，目录+文件名+后缀
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, String localChunkFilePath) {
        ...
        // 将分块文件上传到minio
        boolean success = addMediaFilesToMinio(localChunkFilePath, mimeType, bucket_video, chunkFilePath);
        if (!success) {
    ...
    public RestResponse mergechunk(Long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
        ...
        // 检验合并后的和源文件是否一致，视频上传才成功
        // 先下载合并后的和源文件是否一致，视频上传才成功
    ...
    private String getFilePathByMd5(String fileMd5, String fileExt) {
        return fileMd5.substring(0, 1) + "/" + fileExt.substring(1, 2) + "/" + fileExt + fileExt;
    }
}
